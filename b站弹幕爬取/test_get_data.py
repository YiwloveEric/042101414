from maincode import get_page_url,get_inter_urls,get_data
...
#取第一页的第一个视频的弹幕
#取第一页的视频地址
firstPageUrl = get_page_url(10)[0]
#取第一个视频的地址
firstVideoUrl = get_inter_urls(url=firstPageUrl,headers=header,cookies=dic_cookies)[0]
#取第一个视频的弹幕，并存储于excel表格
get_data(url=firstVideoUrl,headers=header,cookies=dic_cookies)
...
firstPageUrl = get_page_url(10)[0]
#取第二个视频的地址
secondVideoUrl = get_inter_urls(url=firstPageUrl,headers=header,cookies=dic_cookies)[1]
#取第二个视频的弹幕，并存储于excel表格
get_data(url=secondVideoUrl,headers=header,cookies=dic_cookies)
end = time.time()
print(end-start)